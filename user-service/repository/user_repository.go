...
type UserRepositoryInterface interface {
	...
	GetUserByRoleID(ctx context.Context, assignRoleID uint) (*model.UserRole, error)
...
// ────────────────────────────────────────────────────────────────
// AssignUserToRole implements UserRepository Interface
// ────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────
// CreateUser implements UserRepository Inteface
// ────────────────────────────────────────────────────────────────
func (u *userRepository) CreateUser(ctx context.Context, user model.User) (*model.User, error) {
	...
	if user.ID == 0 {
		// nolint:staticcheck // Keep this as a safety guard, though GORM usually auto-fills ID.
...
func (u *userRepository) GetAllUsers(ctx context.Context, page, limit int, search, sortBy, sortOrder string) ([]model.User, int64, error) {
	...
	// ✅ Mulai query dengan model agar GORM tahu tabel yang dipakai
	query := u.db.Model(&model.User{})

	// ✅ Perbaiki syntax pencarian
	if search != "" {
	...
	// ✅ Ambil data dengan pagination dan preload relasi Roles
...
// ────────────────────────────────────────────────────────────────
// GetUserRoleByID Implements UserRepository Interface
// ────────────────────────────────────────────────────────────────
func (u *userRepository) GetUserByRoleID(ctx context.Context, assignRoleID uint) (*model.UserRole, error) {
	select {
	case <-ctx.Done():
