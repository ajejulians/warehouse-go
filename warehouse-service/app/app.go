...
func RunServer() {
	...
	if container.RabbitMQConsumer != nil {
		...
		if err := container.RabbitMQConsumer.StartCounsuming(ctx); err != nil {
			log.Errorf("Failed to start RabbitMQConsumer")
		} else {
			log.Infof("RabbitMQConsumer started successfully")
		}
	}
