...
func (w *warehouseRepository) DeleteWarehouse(ctx context.Context, id uint) error {
	...
	default:
		...
		if len(modelWarehouse.WarehouseProducts) > 0 {
			log.Errorf("[WarehouseProject] DeleteWarehoouse- 3: %v", errors.New("warehouse has products"))
...
func (w *warehouseRepository) GetAllWarehouse(ctx context.Context, page int, limit int, search string, sortBy string, sortOrder string) ([]model.Warehouse, int64, error) {
	...
	default:
		...
		if search != "" {
			query = query.Where("name ILIKE ? OR address ILIKE ? OR phone ILIKE ?", "%"+search+"%", "%"+search+"%")
		}
		...
		if err := query.
			...
			log.Errorf("[WarehoiseRepository] GetAllWarehouses - 3: %v", err)
			return nil, 0, err
		}
...
func (w *warehouseRepository) UpdateWarehouse(ctx context.Context, warehouse *model.Warehouse) error {
	...
	default: 
		...
		if err := w.db.WithContext(ctx).Where("id = ?", warehouse.ID).Preload("WarehouseProducts").First(&existingWarehouse).Error; err != nil {
			log.Errorf("[WarehouseRepository] UpdataWarehouse - 2: %v", err)
			return err
		}
